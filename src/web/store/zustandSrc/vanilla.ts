...
const createStoreImpl: CreateStoreImpl = createState => {
  console.log('createState', createState); // is a fn like () => ({ ...DEFAULT_STORE })
  type TState = ReturnType<typeof createState>; // get ReturnType of () => ({ ...DEFAULT_STORE })
  type Listener = (state: TState, prevState: TState) => void; // 监听 fn
  let state: TState; // let state
  const listeners: Set<Listener> = new Set(); // listeners is a Set
  ...
  const getState: () => TState = () => state; // getState like this: const {name} = store.getState();
  ...
  const destroy: () => void = () => listeners.clear(); // clear Set
